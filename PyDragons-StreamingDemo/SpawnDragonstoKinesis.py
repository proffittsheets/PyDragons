...
#                             DESIGN VARIABLES                                 #
################################################################################
eventsTick = 200           # Number of ticks to loop through to create events
...
#                            AWS SETUP & FUNCTIONS                             #
################################################################################
# Amazon Kinesis Stream Setup
DragonsStream = 'PyDragons-Dragons-Stream'
DragoneventsStream = 'PyDragons-Dragonevents-Stream'
client = boto3.client('kinesis')

# Send Dragon Created Event to Kinesis
def sendDragonCreated(dragon):
    response = client.put_record(
        StreamName=DragonsStream,
        PartitionKey=dragon['dragon_id'],
        Data=json.dumps(dragon)
    )
    print('Dragon Created:' + json.dumps(dragon))

# Send Dragon Action Event to Kinesis
def sendActionCreated(action):
    response = client.put_record(
        StreamName=DragoneventsStream,
        PartitionKey=action['dragon_id'],
        Data=json.dumps(action)
    )
    print('Dragon Action:' + json.dumps(action))
...
def create_dragon():
    ...
    dragon = {}
    dragon['dragon_id'] = str(uuid4())
    dragon['event_uuid'] = str(uuid4())
    dragon['event_timestamp'] = getCurrentUTCTime()
    dragon['utc_offset'] = getOffset()
    dragon['event_name'] = 'dragonSpawned'
...
def create_dragon_action():
    ...
    action = {}
    action['dragon_id'] = dragon['dragon_id']
    action['event_uuid'] = str(uuid4())
    action['event_timestamp'] = getCurrentUTCTime()
    action['utc_offset'] = getOffset()
    action['event_name'] = 'dragonAction'
    action['name'] = dragon['name']
    action['type'] = dragon['type']
    action['state'] = state
    action['value'] = getValue(state, burnMin, burnMax)
    action['value_type'] = getValueType(state, foodTypes, lootTypes)
    action['location'] = Town
    action['affected_pop'] = getAffected(state, TownParam["Pop"])
...
def spawn_dragon_machine():
    """Iterates over the create dragons, dragon actions, and cage functions."""
